...
class myNet():
    ...
    def update(self, x, y_truth):
        ...
        for j in range(self.m):
            self.da[j] = (2 / self.m) * np.sum(self.e * dg(self.z) * self.h[:, j].reshape(self.batch_size)) # dL/da[j] の計算
            self.dw[j] = (2 * self.a[j] / self.m) * np.sum(self.e * dg(self.z) * dg(self.u[:, j].reshape(self.batch_size)) * x) # dL/dw[j] の計算
